...
const discoverItems = [
  {
    ...
    href: "/find-suppliers",
    description: "Guided sourcing: find suppliers, send RFQs, compare offers",
    icon: Sparkles,
    featured: true,
  },
  {
    title: "AI Assistant",
    href: "/assistant",
    description: "Chat with AI for sourcing help and questions",
    icon: MessageSquare,
  },
  {
...
const platformItems = [
  ...
  },
  {
    ...
    description: "Offer your services to importers and manufacturers",
    icon: Briefcase,
  },
  {
    title: "Pricing",
...
export function Header() {
  ...
  const getDashboardUrl = () => {
    ...
    switch (user.role) {
      ...
      case "service-provider": return "/dashboard/service-provider"
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="mx-auto flex h-14 sm:h-16 max-w-7xl items-center justify-between px-3 sm:px-6 lg:px-8">
        ...
        <Link href="/" className="flex items-center gap-1.5 sm:gap-2">
          <div className="flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-lg bg-primary">
            <Factory className="h-3.5 w-3.5 sm:h-5 sm:w-5 text-primary-foreground" />
          </div>
          <span className="font-serif text-base sm:text-xl font-medium text-foreground">SourceNest</span>
        </Link>
        ...
        <NavigationMenu className="hidden lg:flex">
          <NavigationMenuList>
            ...
            <NavigationMenuItem>
              <NavigationMenuLink asChild>
                <Link href="/service-providers" className={cn(navigationMenuTriggerStyle(), "bg-transparent")}>
                  Service Providers
                </Link>
              </NavigationMenuLink>
            </NavigationMenuItem>
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 lg:hidden">
          <Button variant="ghost" size="icon" aria-label="Saved Items" className="relative h-8 w-8 sm:h-9 sm:w-9" asChild>
            <Link href="/dashboard/buyer/saved">
              <Heart className={cn("h-4 w-4 sm:h-5 sm:w-5", totalSaved > 0 && "text-rose-500")} />
              {totalSaved > 0 && (
                <span className="absolute -right-0.5 -top-0.5 flex h-3.5 w-3.5 sm:h-4 sm:w-4 items-center justify-center rounded-full bg-rose-500 text-[9px] sm:text-[10px] font-medium text-white">
                  {totalSaved > 9 ? "9+" : totalSaved}
                </span>
              )}
            </Link>
          </Button>
          <Button variant="ghost" size="icon" aria-label="Messages" className="relative h-8 w-8 sm:h-9 sm:w-9" asChild>
            <Link href="/messages">
              <MessageSquare className="h-4 w-4 sm:h-5 sm:w-5" />
              <span className="absolute right-1 top-1 h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full bg-secondary" />
            </Link>
          </Button>
          <Button variant="ghost" size="icon" aria-label="Search" className="h-8 w-8 sm:h-9 sm:w-9" asChild>
            <Link href="/search">
          ...
          <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
            ...
            <SheetContent side="right" className="w-[85vw] max-w-xs overflow-y-auto p-4 sm:p-6">
              <SheetHeader>
                <SheetTitle className="flex items-center gap-1.5">
                  <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary">
                    <Factory className="h-3.5 w-3.5 text-primary-foreground" />
                  </div>
                  <span className="font-serif text-base">SourceNest</span>
                </SheetTitle>
              </SheetHeader>
              <nav className="mt-4 flex flex-col gap-0.5">
                ...
                <button
                  ...
                  className="flex w-full items-center justify-between rounded-md px-2.5 py-2 text-sm font-medium hover:bg-muted"
                >
                  Discover
                  <ChevronDown className={cn("h-3.5 w-3.5 transition-transform", expandedSection === 'discover' && "rotate-180")} />
                </button>
                ...
                >
                  Platform
                  <ChevronDown className={cn("h-3.5 w-3.5 transition-transform", expandedSection === 'platform' && "rotate-180")} />
                </button>
                ...
                >
                  Resources
                  <ChevronDown className={cn("h-3.5 w-3.5 transition-transform", expandedSection === 'resources' && "rotate-180")} />
                </button>
                ...
                {isAuthenticated && user ? (
                  <>
                    <div className="mb-1.5 flex items-center gap-2.5 px-2.5 py-1.5">
                      <Avatar className="h-8 w-8">
                    ...
                    <Link
                      ...
                      onClick={() => setMobileOpen(false)}
                      className="flex items-center gap-2 rounded-md px-2.5 py-1.5 text-sm font-medium hover:bg-muted"
                    >
                    ...
                    <button
                      ...
                      className="flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-sm font-medium text-destructive hover:bg-muted"
                    >
                  ...
                  <>
                    ...
                    <Button className="mt-2 w-full h-9 text-sm" asChild>
