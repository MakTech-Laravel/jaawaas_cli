...
import { Eye, EyeOff, Loader2, Users, Factory, Check, AlertCircle, Upload, FileText, X, Camera, Globe, Building2, Info, Briefcase, Linkedin, Award, FolderOpen } from "lucide-react"
import { Separator } from "@/components/ui/separator"

export default function SignUpPage() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const defaultRole = searchParams.get("role") as "buyer" | "manufacturer" | "service-provider" | null
  ...
  const [formData, setFormData] = useState({
    role: defaultRole || ("buyer" as "buyer" | "manufacturer" | "service-provider"),
    firstName: "",
    lastName: "",
    email: "",
    company: "",
    ...
    additionalNotes: "",
    // Service provider review fields
  ...
  const handleBusinessRegistrationChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (file) {
  ...
  const handlePortfolioFilesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files || [])
    const newFiles = [...formData.portfolioFiles, ...files].slice(0, 8) // Max 8 files
    setFormData({ ...formData, portfolioFiles: newFiles })
  }
  ...
  const handleRoleSelect = (role: "buyer" | "manufacturer" | "service-provider") => {
        ...
        <div className="mt-8 space-y-4">
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-foreground">I&apos;m a Service Provider</h3>
                <p className="mt-1 text-sm text-muted-foreground">
                  Offer design, logistics, customs, inspection, or consulting services
                </p>
                <ul className="mt-3 space-y-1 text-sm text-muted-foreground">
                  <li className="flex items-center gap-2">
                    <Check className="h-4 w-4 text-secondary" />
                    Reach importers & manufacturers
                  </li>
                  <li className="flex items-center gap-2">
                    <Check className="h-4 w-4 text-secondary" />
    ...
    <div>
      <div className="text-center lg:text-left">
        ...
        <p className="mt-2 text-muted-foreground">
          {formData.role === "buyer"
            ...
            : formData.role === "service-provider"
              ? "Complete registration, then choose a package to get listed"
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-6">
        <div className="space-y-4">
          ...
          {/* Service Provider Review Section */}
          {formData.role === "service-provider" && (
            <>
              <Separator className="my-6" />
              ...
              <div className="rounded-lg border border-secondary/30 bg-secondary/5 p-4">
                <div className="flex items-start gap-3">
                  <Info className="h-5 w-5 text-secondary mt-0.5" />
                  <div>
                    <h3 className="font-medium text-foreground">Provider Review Required</h3>
                    <p className="mt-1 text-sm text-muted-foreground">
                      To earn the <strong>Reviewed Service Provider</strong> badge, share a few details about your
                      business. Our team reviews your information and supporting documents before your profile is
                      approved.
                    </p>
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="businessRegistration" className="flex items-center gap-1">
                  <FileText className="h-4 w-4 text-secondary" />
                  Business Registration <span className="text-destructive">*</span>
                </Label>
                <p className="text-xs text-muted-foreground mb-2">
                  Upload your business registration or company incorporation document (PDF, JPG, PNG - max 10MB)
                </p>
                <input
                ...
                {formData.businessRegistration ? (
                  <div className="flex items-center justify-between rounded-lg border border-secondary/30 bg-secondary/5 p-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-foreground">{formData.businessRegistration.name}</p>
                        <p className="text-xs text-muted-foreground">
                          {(formData.businessRegistration.size / 1024 / 1024).toFixed(2)} MB
                        </p>
                      </div>
                    </div>
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      onClick={() => setFormData({ ...formData, businessRegistration: null })}
                      disabled={isLoading}
                    >
                      <X className="h-4 w-4" />
                    </Button>
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="w-full h-20 border-dashed"
                    onClick={() => businessRegistrationRef.current?.click()}
                    disabled={isLoading}
                  >
                    <div className="flex flex-col items-center gap-2">
                      <Upload className="h-5 w-5 text-muted-foreground" />
                      <span className="text-sm text-muted-foreground">Click to upload business registration</span>
                    </div>
                  </Button>
                )}
              </div>
              ...
              {/* City Field */}
              <div className="space-y-2">
                <Label htmlFor="sp-city" className="flex items-center gap-1">
                  <Building2 className="h-4 w-4 text-secondary" />
                  City <span className="text-destructive">*</span>
                </Label>
                <Input
                  id="sp-city"
                  placeholder="e.g., London, Singapore, Tel Aviv"
                  value={formData.city}
                  onChange={(e) => setFormData({ ...formData, city: e.target.value })}
                  required
                  disabled={isLoading}
                />
                <p className="text-xs text-muted-foreground">
                  Combined with country above, this forms your business location
                </p>
              </div>

              {/* Company Website */}
              <div className="space-y-2">
                <Label htmlFor="sp-website" className="flex items-center gap-1">
                  <Globe className="h-4 w-4 text-secondary" />
                  Company Website <span className="text-muted-foreground text-xs">(optional)</span>
                </Label>
                <Input
              ...
              {/* LinkedIn Profile */}
              <div className="space-y-2">
                <Label htmlFor="sp-linkedin" className="flex items-center gap-1">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground mb-2">
                  Upload samples of your work, case studies, or project files (PDF, images, max 8 files)
                </p>
                <input
                  ...
                  className="hidden"
                  disabled={isLoading || formData.portfolioFiles.length >= 8}
                />
                ...
                {formData.portfolioFiles.length < 8 && (
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
                    onClick={() => portfolioFilesRef.current?.click()}
                    disabled={isLoading}
                  >
                    <Upload className="mr-2 h-4 w-4" />
                    Add Portfolio Files ({formData.portfolioFiles.length}/8)
                  </Button>
                )}
              </div>
              ...
              <div className="space-y-2">
                <Label htmlFor="sp-certifications" className="flex items-center gap-1">
                  <Award className="h-4 w-4 text-secondary" />
                  Professional Certifications <span className="text-muted-foreground text-xs">(optional)</span>
                </Label>
                <Textarea
                  id="sp-certifications"
                  placeholder="List any relevant certifications, licenses, or accreditations (e.g., licensed customs broker, ISO 9001, certified translator)..."
              ...
              {/* Additional Notes */}
              <div className="space-y-2">
                <Label htmlFor="sp-notes" className="flex items-center gap-1">
                  Additional Notes <span className="text-muted-foreground text-xs">(optional)</span>
                </Label>
                <Textarea
                  id="sp-notes"
                  placeholder="Tell us more about the services you offer, your experience, and the clients you work with..."
                  value={formData.additionalNotes}
                  onChange={(e) => setFormData({ ...formData, additionalNotes: e.target.value })}
                  disabled={isLoading}
                  rows={3}
        ...
        {(() => {
          const manufacturerFieldsValid =
            formData.role === "manufacturer"
              ? !!(formData.businessLicense && formData.city.trim() !== "")
              : true
          const serviceProviderFieldsValid =
            formData.role === "service-provider"
              ? !!(
                  formData.businessRegistration &&
                  formData.city.trim() !== "" &&
              ...
              : true
          const needsReview = formData.role === "manufacturer" || formData.role === "service-provider"
          const isSubmitDisabled =
            isLoading || !formData.agreeTerms || !manufacturerFieldsValid || !serviceProviderFieldsValid

          return (
            <>
              <Button
                type="submit"
                className={cn("w-full", needsReview && "bg-secondary text-secondary-foreground hover:bg-secondary/90")}
              ...
              {needsReview && (
                <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                  <p className="text-center text-xs text-amber-800">
                    <strong>Important:</strong> Your {formData.role === "manufacturer" ? "manufacturer" : "service provider"}{" "}
                    account will be pending admin approval after registration. Our team will review your information and
                    documents and notify you once approved. This typically takes 1-2 business days.
                  </p>
                </div>
              )}
            </>
